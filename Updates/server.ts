...
async function startServer() {
  ...
  app.get('/api/trends', (req, res) => {
    res.json([
      ...
      { id: '2', title: 'Best CD Rates for April', volume: 'Medium', trend: 'stable' },
      { id: '3', title: 'Mortgage Refinance Strategies', volume: 'High', trend: 'up' },
      { id: '4', title: 'Inflation Impact on Savings', volume: 'Medium', trend: 'down' },
    ]);
  });
  ...
  app.get('/api/stories', async (req, res) => {
    ...
    try {
      let q;
      if (slug) {
        q = query(collection(db, 'stories'), where('slug', '==', slug), limit(1));
      } else if (type === 'top') {
        q = query(collection(db, 'stories'), where('isTopStory', '==', true), limit(1));
      } else if (category) {
        q = query(collection(db, 'stories'), where('category', '==', category), orderBy('publishedAt', 'desc'), limit(10));
      } else {
        q = query(collection(db, 'stories'), orderBy('publishedAt', 'desc'), limit(20));
      }
