...
import { Send, Bot, User, Sparkles, Loader2, X, ChevronDown } from 'lucide-react';
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      <motion.div
        ...
        animate={{
          ...
          marginBottom: isMinimized ? 0 : 16
        }}
        transition={{ type: 'spring', damping: 25, stiffness: 200 }}
        className="w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden origin-bottom-right"
      >
      ...
      <motion.button
        ...
        className={cn(
          ...
          isMinimized ? "bg-brand-600 text-white" : "bg-white text-slate-600 border border-slate-200"
        )}
      >
        {isMinimized ? <Bot className="w-7 h-7" /> : <ChevronDown className="w-7 h-7" />}
      </motion.button>
    </div>
  );
}
