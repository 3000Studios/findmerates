...
import { Newspaper, Clock, ArrowRight, MessageSquare, ChevronRight, User, Send, Share2, Bookmark, Star, ShieldCheck, Loader2 } from 'lucide-react';
...
export default function StoryDetail() {
  ...
  useEffect(() => {
    if (!story) return;
    const q = query(collection(db, 'stories', story.id, 'comments'), orderBy('createdAt', 'desc'));
  ...
  if (isLoading) return (
    <div className="min-h-screen flex items-center justify-center bg-brand-900">
      <div className="text-center">
        ...
        <p className="text-accent-gold font-bold uppercase tracking-[0.5em] text-[10px]">Accessing Intelligence Partition</p>
      </div>
    </div>
  );
  
  if (!story) return <div className="min-h-screen flex items-center justify-center text-white font-bold uppercase tracking-widest">Story not found.</div>;
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-20">
          ...
          >
            <nav className="flex items-center justify-center gap-4 text-[10px] font-bold text-accent-gold mb-12 uppercase tracking-[0.5em]">
              ...
              <Link to="/stories" className="hover:text-white transition-colors">Intelligence</Link>
              <ChevronRight className="w-3 h-3" />
            ...
            <div className="flex items-center justify-center gap-12 text-[10px] font-bold text-slate-500 uppercase tracking-[0.4em]">
              <span className="flex items-center gap-3"><User className="w-4 h-4 text-accent-gold" /> {story.author}</span>
              <span className="flex items-center gap-3"><Clock className="w-4 h-4 text-accent-gold" /> {new Date(story.publishedAt).toLocaleDateString()}</span>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-1">
            ...
            <section className="mt-32 space-y-1">
              <div className="bg-brand-600 p-12 border border-white/5 flex items-center justify-between">
                <h3 className="text-2xl font-display font-bold text-white flex items-center gap-6 uppercase tracking-tight">
                  <MessageSquare className="w-6 h-6 text-accent-gold" /> Analysis Discussion ({comments.length})
                </h3>
              </div>
              ...
              {auth.currentUser ? (
                ...
                <div className="bg-brand-600 border border-white/5 p-24 text-center">
                  <p className="text-slate-600 mb-12 font-bold uppercase tracking-[0.4em] text-[10px]">Institutional authentication required</p>
                  <button 
                  ...
                  >
                    ...
                    <div className="flex-grow">
                      <div className="flex items-center justify-between mb-6">
                        <h4 className="font-bold text-white uppercase tracking-widest text-xs">{comment.displayName}</h4>
                        <span className="text-[9px] font-bold text-slate-700 uppercase tracking-[0.4em]">
                          {comment.createdAt && (comment.createdAt as any).toDate ? (comment.createdAt as any).toDate().toLocaleDateString() : 'Just now'}
                        </span>
                      </div>
                      <p className="text-slate-500 text-lg leading-relaxed font-medium">{comment.text}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <div className="lg:col-span-4 space-y-1">
            ...
            <div className="bg-brand-600 border border-white/5 p-12">
              <h3 className="text-xl font-display font-bold text-white mb-12 uppercase tracking-tight">Related Intel</h3>
                ...
                {[1, 2, 3].map(i => (
                  <div key={i} className="group flex gap-8 cursor-pointer">
                    <div className="w-24 h-24 bg-brand-900 border border-white/5 overflow-hidden shrink-0">
                      <img src={`https://picsum.photos/seed/rel${i}/300/300`} alt="Related" className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" referrerPolicy="no-referrer" />
                    </div>
                    <div className="flex flex-col justify-center">
                      <h4 className="font-bold text-white text-sm leading-tight group-hover:text-accent-gold transition-all uppercase tracking-tight group-hover:scale-[1.05] origin-left">Strategic Market Shift Analysis</h4>
                      <p className="text-[9px] font-bold text-slate-700 mt-4 uppercase tracking-widest">2 hours ago</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-accent-gold p-12">
              <h3 className="text-brand-900 text-xl font-display font-bold mb-8 uppercase tracking-tight">Pro Intelligence</h3>
              <p className="text-brand-900/70 text-sm font-bold uppercase tracking-widest mb-12 leading-relaxed">Unlock real-time predictive modeling and institutional alerts.</p>
              <Link to="/pro" className="w-full btn-corporate bg-brand-900 text-white border-none flex items-center justify-center gap-4">
